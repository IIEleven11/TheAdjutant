...
driver.get('https://challonge.com/communities/MallkusMasters/tournaments')
today = datetime.now()
...
def create_event_if_not_exists(service, calendar_id, event):
    ...
    if not any(e['summary'] == event['summary'] and e['start']['dateTime'] == event['start']['dateTime'] for e in events):
...
for title, date_str, time_str, link in zip(event_titles, event_dates, event_times, event_links):
    ...
    time_str_no_tz = time_str.replace(' -05', '')
