...
driver.get('https://challonge.com/communities/a0a85463fe059b15da0528ad/tournaments')
...
for title, date_str, time_str, link in zip(event_titles, event_dates, event_times, event_links):
    ...
    time_str_no_tz = time_str.replace(' CDT', '')
    ...
    if any(event['summary'] == title for event in events):
        ...
        try:
            ...
            print("An error occurred: ", e)
            print(driver.page_source) # add this line to print the current page source when an error occurs {e}")
